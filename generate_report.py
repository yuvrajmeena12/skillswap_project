...
def normalize_author(author):
    """
    Converts different Git author names belonging to the same person
    into one canonical name for the report.
    ...
    # ---------------------------------------------------------
    # ADD ALL ALIASES OF THE SAME PERSON HERE
    # ---------------------------------------------------------
    author_aliases = {
        "yuvrajmeena12": "yuvrajmeena12",
        "yuvraj meena": "yuvrajmeena12",
        "yuvraj  meena": "yuvrajmeena12",
    }

    # If author matches a known alias, return canonical name
    if normalized in author_aliases:
        return author_aliases[normalized]

    # Otherwise keep the original author's name
    return author
...
def get_git_metrics(interval="weekly"):
    ...
    IMPORTANT:
    Author names are normalized before being used as dictionary keys.
    Therefore multiple Git names belonging to the same person are
    ...
    for line in raw_output.strip().split('\n'):
        ...
        if line.startswith('COMMIT|||'):
            ...
            # yuvrajmeena12
            #
            # Both therefore use the SAME dictionary key.
            # -------------------------------------------------
...
def create_charts(
    ...
    if authors:
        ...
        # repeat the available colors safely.
        bar_colors = [
